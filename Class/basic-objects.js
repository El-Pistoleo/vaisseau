...
function pave2(side, offset) {
   ...
   // normalTemp = calculenormale([-sOff,-sOff,sOff], [sOff,-sOff,sOff], [sOff,sOff,sOff]);
   // face( [-sOff,-s,sOff, sOff,-s,sOff, sOff,s,sOff, -sOff,s,sOff], normalTemp );
   //
   // normalTemp = calculenormale([-s,-s,-s], [-s,s,-s], [s,s,-s]);
   // face( [-s,-s,-s, -s,s,-s, s,s,-s, s,-s,-s], normalTemp);
   //
   // normalTemp = calculenormale([-s,s,-s], [-sOff,sOff,sOff], [sOff,sOff,sOff]);
   // face( [-s,s,-s, -sOff,s,sOff, sOff,s,sOff, s,s,-s], normalTemp);
   //
   // normalTemp = calculenormale([-s,-s,-s], [s,-s,-s], [sOff,-sOff,sOff]);
   // face( [-s,-s,-s, s,-s,-s, sOff,-s,sOff, -sOff,-s,sOff], normalTemp );
   //
   // normalTemp = calculenormale([s,-s,-s], [ s,s,-s], [sOff,sOff,sOff]);
   // face( [s,-s,-s, s,s,-s, sOff,s,sOff, sOff,-s,sOff], normalTemp );
   //
   ...
   normalTemp = calculenormale([-s,-sOff,s], [s,-sOff,s], [s,sOff,s]);
   face( [-s,-sOff,s, s,-sOff,s, s,sOff,s, -s,sOff,s], normalTemp );

   normalTemp = calculenormale([-s,-s,-s], [-s,s,-s], [s,s,-s]);
   face( [-s,-s,-s, -s,s,-s, s,s,-s, s,-s,-s], normalTemp);

   normalTemp = calculenormale([-s,s,-s], [-s,sOff,s], [s,sOff,s]);
   face( [-s,s,-s, -s,sOff,s, s,sOff,s, s,s,-s], normalTemp);

   normalTemp = calculenormale([-s,-s,-s], [s,-s,-s], [s,-sOff,s]);
   face( [-s,-s,-s, s,-s,-s, s,-sOff,s, -s,-sOff,s], normalTemp );
...
function pave(side, offset) {
   ...
   normalTemp = calculenormale([-sOff,-sOff,sOff], [sOff,-sOff,sOff], [sOff,sOff,sOff]);
   face( [-sOff,-sOff,sOff, sOff,-sOff,sOff, sOff,sOff,sOff, -sOff,sOff,sOff], normalTemp );

   normalTemp = calculenormale([-s,-s,-s], [-s,s,-s], [s,s,-s]);
   face( [-s,-s,-s, -s,s,-s, s,s,-s, s,-s,-s], normalTemp);

   normalTemp = calculenormale([-s,s,-s], [-sOff,sOff,sOff], [sOff,sOff,sOff]);
   face( [-s,s,-s, -sOff,sOff,sOff, sOff,sOff,sOff, s,s,-s], normalTemp);

   normalTemp = calculenormale([-s,-s,-s], [s,-s,-s], [sOff,-sOff,sOff]);
   face( [-s,-s,-s, s,-s,-s, sOff,-sOff,sOff, -sOff,-sOff,sOff], normalTemp );
...
function largeTriangle(h, angle, deep){
  ...
  newHy = sinAngle*h;
        ...
        //Premier point //Deuxieme point hyoptenuse
  normalTemp = calculenormale([newHx ,h,h], [h,-h,h], [-h,-h,h]);
  face( [newHx ,h,h, h,-h,h, -h,-h,h ], normalTemp ); //Triangle arriere

  normalTemp = calculenormale([-h,-h,h+deep], [h,-h,h+deep], [newHx,h,h+deep]);
  face( [-h,-h,h+deep, h,-h,h+deep, newHx,h,h+deep ], normalTemp ); //Triangle avamt

  normalTemp = calculenormale([newHx,h,h+deep], [newHx,h,h], [-h,-h,h]);
  face2( [newHx,h,h+deep, newHx,h,h, -h,-h,h ,-h,-h,h+deep], normalTemp ); //Face hypotenuse

  normalTemp = calculenormale([h,-h,h+deep], [h,-h,h], [newHx,h,h]);
  face2( [h,-h,h+deep, h,-h,h, newHx,h,h, newHx,h,h+deep], normalTemp ); //Face cote

  normalTemp = calculenormale([-h,-h,h+deep], [-h,-h,h], [h,-h,h]);
  face2( [-h,-h,h+deep, -h,-h,h ,h,-h,h, h,-h,h+deep  ], normalTemp ); //Face dessous
...
function tetra(s){
  ...
  function face(xyz, nrm) {
     ...
     indices.push(start,start+1,start+2);
        ...
        //Premier point //Deuxieme point hyoptenuse
  normalTemp = calculenormale([0,s,0], [0,0,0], [0,0,s]);
  face( [0,s,0, 0,0,0, 0,0,s], normalTemp ); //Triangle arriere

  normalTemp = calculenormale( [0,0,0], [0,s,0], [s,0,0]);
  face( [ 0,0,0, 0,s,0, s,0,0], normalTemp ); //Triangle arriere

  normalTemp = calculenormale([0,s,0], [0,0,s], [s,0,0]);
  face( [ 0,s,0, 0,0,s, s,0,0], normalTemp ); //Triangle arriere

  normalTemp = calculenormale([s,0,0], [0,0,s], [0,0,0]);
  face( [s,0,0, 0,0,s, 0,0,0], normalTemp ); //Triangle arriere
...
function uvSphereCoupe(radius, slices, stacks) {
   radius = radius || 0.5;
   slices = slices || 32;
   stacks = stacks || 16;
   var vertexCount = (slices+1)*(stacks+1);
